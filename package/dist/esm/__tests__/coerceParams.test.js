...
import coerceParams, { defaultCoerceBoolean, defaultCoerceNumber, defaultCoerceString, defaultCoerceBigInt, defaultCoerceDate, } from "../coerceParams";
...
describe("coerceParams", () => {
    it("ZodDate with ZodMetadata", () => {
        expect(coerceParams(z.date().withMetadata({ foo: "bar" })).parse("2021-03-03")).toEqual(new Date("2021-03-03"));
    });
    ...
    it("ZodNumber in ZodSet", function () {
        expect(coerceParams(z.set(z.number())).parse(new Set(["123"]))).toEqual(new Set([123]));
    });
    ...
    it("ZodNumber in ZodOptional", function () {
        ...
        expect(coerceParams(z.number().optional()).parse(undefined)).toEqual(undefined);
    });
    ...
    it("ZodNumber in ZodEffects", function () {
        expect(coerceParams(z.number().transform((x) => x * 2)).parse("123")).toEqual(246);
    });
    it("ZodPipeline", function () {
        expect(() => coerceParams(z.string().pipe(z.number())).parse(456)).toThrow();
        expect(coerceParams(z.number().pipe(z.number().max(5))).parse("5")).toEqual(5);
        expect(() => coerceParams(z.number().pipe(z.number().max(5))).parse("6")).toThrow();
    });
    it("ZodMap", function () {
        expect(coerceParams(z.map(z.number(), z.number())).parse(new Map([["1", "2"]]))).toEqual(new Map([[1, 2]]));
    });
    it("ZodDiscriminatedUnion", function () {
        const schema = coerceParams(z.discriminatedUnion("type", [
    ...
    it("ZodUnion", function () {
        const schema = coerceParams(z.union([
            z.object({ type: z.literal("a"), a: z.number() }),
            z.object({ type: z.literal("b"), b: z.string() }),
        ]));
        expect(schema.parse({ type: "a", a: "1" })).toEqual({ type: "a", a: 1 });
    ...
    it("ZodUnion of ZodLiterals", function () {
        const schema = coerceParams(z.union([z.literal(1), z.literal(2), z.literal(3), z.literal("123")]));
    ...
    it("ZodIntersection", function () {
        const schema = coerceParams(z.intersection(z.object({ a: z.number() }), z.object({ a: z.number(), b: z.string() })));
