...
function makeUrl(callPath, outputCase = "kebab") {
    return outputCase === "kebab"
        ? callPath.map(strings_1.kebabCase).join("/")
        : callPath.join("/");
}
/**
...
function makeRequest(config, action, callPath, body) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const options = method !== "GET" && body !== undefined
            ...
            : { method };
        console.log("Making request", { url, method, body, options });
        console.log("Making request", { fetchFn });
        const response = yield fetchFn(url, options);
        console.log("Making request", { response });
