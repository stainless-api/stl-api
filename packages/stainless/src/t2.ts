...
class ShortString extends Schema implements StringSchemaProps {
...
type schemaTypeToZod<T extends Schema> = T extends {
  ...
  ? z.ZodMetadata<
      toZod<Omit<T, "prismaModel">>,
      { stainless: { prismaModel: M } }
    >
  : T extends { [prismaModelLoader]: true; input: infer I; model: infer M }
  ? z.ZodEffects<toZod<I>, { TODO: true }, input<I>>
  : T extends {
...
const prismaModelLoader = Symbol("prismaModelLoader");

abstract class PrismaModelLoader extends Schema {
  declare [prismaModelLoader]: true;
...
class User extends Schema implements HasPrismaModel {
  declare input: {
    id: string;
    email: string;
