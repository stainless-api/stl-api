...
exports.getPropertyDeclaration = exports.convertType = exports.convertSymbol = exports.processModuleIdentifiers = exports.ConvertTypeContext = exports.SchemaGenContext = exports.ErrorAbort = void 0;
...
function prefixValueWithModule(ctx, name, currentFilePath, valueFilePath, importFromUserFile, escapedName) {
    ...
    return ctx.generateInUserFile ||
        ...
        : factory.createPropertyAccessExpression(getModuleIdentifier(currentFileInfo, valueFilePath, importFromUserFile), name);
}
...
generateInline) {
    ...
    switch (baseTypeName) {
        case "Transform": {
            const symbol = ty.getSymbolOrThrow(`failed to get symbol for transform: ${ty.getText()}`);
            const inputType = (_a = ty.getProperty("input")) === null || _a === void 0 ? void 0 : _a.getTypeAtLocation(ctx.node);
            if (!inputType)
                throw new Error("Encountered Transform class without required input property");
            ...
            return methodCall(convertType(ctx, inputType, diagnosticItem), "transform", [
                factory.createPropertyAccessExpression(factory.createNewExpression(prefixValueWithModule(ctx, symbol.getName(), ctx.currentFilePath, typeFilePath, true), undefined, []), factory.createIdentifier("transform")),
            ]);
        }
        case "Refine":
    ...
    switch (baseTypeName) {
        case "PrismaModel":
            return convertPrismaModelType(ctx, ty, typeSymbol, "prismaModel", baseTypeName, diagnosticItem);
        case "PrismaModelLoader":
            return convertPrismaModelType(ctx, ty, typeSymbol, "prismaModelLoader", baseTypeName, diagnosticItem);
    }
    if (ty.isClass()) {
        if (!typeSymbol) {
...
function convertRefineType(ctx, ty, symbol, refineType, diagnosticItem) {
    var _a;
    if (!symbol) {
        ctx.addError(diagnosticItem, { message: `failed to get symbol for ${refineType}: ${ty.getText()}` }, true);
    }
    const inputType = (_a = ty.getProperty("input")) === null || _a === void 0 ? void 0 : _a.getTypeAtLocation(ctx.node);
    if (!inputType)
    ...
    const callArgs = [
        factory.createPropertyAccessExpression(factory.createNewExpression(factory.createIdentifier(symbol.getName()), undefined, []), factory.createIdentifier(refineType)),
    ];
    if (refineType === "refine") {
        callArgs.push(factory.createPropertyAccessExpression(factory.createNewExpression(prefixValueWithModule(ctx, symbol.getName(), ctx.currentFilePath, typeFilePath, true), undefined, []), factory.createIdentifier("message")));
    }
    return methodCall(convertType(ctx, inputType, diagnosticItem), refineType, callArgs);
}
...
function getBaseTypeName(type) {
    ...
    const baseSymbol = extendsTypeNode === null || extendsTypeNode === void 0 ? void 0 : extendsTypeNode.getType().getSymbol();
    return baseSymbol === null || baseSymbol === void 0 ? void 0 : baseSymbol.getName();
}
...
function mangleString(str) {
    ...
    for (const codePointString of str) {
        if (codePointString === path_1.default.sep) {
