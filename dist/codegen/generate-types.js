...
function zodToString(schema) {
    const { node } = (0, zod_to_ts_1.zodToTs)(schema, undefined, { nativeEnums: "union" });
...
function makeActionType(name, actionPath, api, config) {
    ...
    use${(0, strings_1.capitalize)((0, strings_1.camelCase)(name))}(${body}${query}): {
      queryFn(): Promise<${returnType}>;
      queryKey: string[];
    };`);
    if (config.extensions) {
        const input = action.body ? zodToString(action.body) : undefined;
        const extensionMethds = (0, dedent_js_1.default) `
    ...
    `;
        const extensionMutationMethod = input !== undefined
            ? (0, dedent_js_1.default) `useMutation(opts?: UseMutationOptions<${returnType}, unknown, ${input}>): ReactQuery.UseMutationResult<${returnType}, unknown, ${input}>;`
            : (0, dedent_js_1.default) `useMutation(opts?: UseMutationOptions<${returnType}, unknown, void>): ReactQuery.UseMutationResult<${returnType}, unknown, void>;`;
        const extensionQueryKeyMethod = `getQueryKey(): string[];`;
        if (body.length || query.length) {
            types.push(`${(0, strings_1.camelCase)(name)}: {
          ...
          ${extensionQueryKeyMethod}
            ...
            types.push((0, dedent_js_1.default) `${(0, strings_1.camelCase)(name)}: {
        (): Promise<${returnType}>;
        ${extensionMethds}
        ${extensionMutationMethod}
        ${extensionQueryKeyMethod}
        ...
        types.push(`${(0, strings_1.camelCase)(name)}(${body}${query}): Promise<`);
...
function generateOutput(api, config, installLocation = "@stl-api/client", reactQueryAlias = "@tanstack/react-query") {
